...
C_MODEL_LIB_PATH := /home/xiaoyu/workspace/riscv-mini/3rdparty/build/lib
C_MODEL_INCLUDE := /home/xiaoyu/workspace/riscv-mini/3rdparty
C_MODEL_LIB := rvcsim
CXXFLAGS += -std=c++14 -Wall -Wno-unused-variable -I$(C_MODEL_INCLUDE) -pthread -g
...
VERILATOR_FLAGS = --assert -Wno-STMTDLY -O0 --trace --threads $(threads)\
	...
	-LDFLAGS "-L$(C_MODEL_LIB_PATH) -l$(C_MODEL_LIB) -Wl,-rpath=$(C_MODEL_LIB_PATH)"
...
test_bin_files = $(wildcard $(base_dir)/tests/*.bin)
test_out_files = $(foreach f,$(test_bin_files),$(patsubst %.bin,%.out,$(out_dir)/$(notdir $f)))

$(test_out_files): $(out_dir)/%.out: $(base_dir)/VTile $(base_dir)/tests/%.bin
	mkdir -p $(out_dir)
	$^ $(patsubst %.out,%.vcd,$@) 2> $@

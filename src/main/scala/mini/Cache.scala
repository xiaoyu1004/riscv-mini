...
class CacheModuleIO(
    nastiParams: NastiBundleParameters,
    addrWidth:   Int,
    dataWidth:   Int
) extends Bundle {
...
class Cache(val p: CacheConfig, val nasti: NastiBundleParameters, val xlen: Int)
    extends Module {
  // local parameters
  val nSets          = p.nSets
  ...
  val wmask =
    Mux(!is_alloc, (cpu_mask << Cat(off_reg, 0.U(byteOffsetBits.W))).zext, -1.S)
  val wdata = Mux(
    !is_alloc,
    ...
    dataMem.zipWithIndex.foreach {
      case (mem, i) =>
        ...
        mem.write(
          idx_reg,
          data,
          wmask((i + 1) * wBytes - 1, i * wBytes).asBools
        )
  ...
  io.nasti.w.bits := NastiWriteDataBundle(nasti)(
    VecInit.tabulate(dataBeats)(i =>
      read((i + 1) * nasti.dataBits - 1, i * nasti.dataBits)
    )(write_count),
    None,
    write_wrap_out
  )
